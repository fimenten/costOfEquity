...
if __name__ == "__main__":
    ...
    parser.add_argument('--freq', type=str, default="W",help='freq to calc beta')

    args = parser.parse_args()

    # print(f'Ticker symbol: {args.ticker}')
    if args.yfinance:
        df_TOPIX = get_topix().resample(args.freq).sum()
        df_ticker = get_ticker_df(args.ticker).resample(args.freq).sum()
